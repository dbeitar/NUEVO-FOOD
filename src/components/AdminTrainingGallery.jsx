...
export default function AdminTrainingGallery() {
  ...
  const fetchGallery = async () => {
    try {
      ...
      console.warn('Error fetching training gallery');
    }
  };
  ...
  const handleAdd = async (e) => {
    ...
    setLoading(true);
    try {
      await api.post('/training/admin/gallery', form);
      await fetchGallery();
      setForm({ name: '', muscle_group: '', youtube_url: '' });
    } catch (e) {
      alert(e?.response?.data?.error || 'Error agregando video a la galería');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('¿Eliminar URL de esta base?')) return;
    try {
  ...
  return (
    <div className="card">
      ...
      <form
        onSubmit={handleAdd}
        className="bg-stone-50 border border-stone-200 p-4 rounded-xl flex flex-col md:flex-row gap-4 items-end mb-6"
      >
        <div className="flex-1 w-full">
          ...
          <input
            ...
            type="text"
            required
            placeholder="SENTADILLA MÁQUINA SMITH"
            value={form.name}
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          ...
          <tbody>
            ...
            {exercises.length === 0 && (
              <tr>
                <td colSpan="4" className="text-center py-6 text-stone-500 text-sm italic">
